...
int main()
{
	...
	Shader cubeShader("shaders/vertex.vert", "shaders/fragment_directional_light.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float light_x = glm::cos(glm::radians((float)glfwGetTime()*10.0f)) * 2.0f;
		float light_z = glm::sin(glm::radians((float)glfwGetTime()*10.0f)) * 2.0f;
		...
		cubeShader.setVec3("light.direction", glm::vec3(light_x, -2.0f, light_z));
		...
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::translate(model, -glm::vec3(light_x, -2.0f, light_z));
